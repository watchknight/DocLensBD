...
const Products: React.FC = () => {
  ...
  const clearFilters = () => {
    setSelectedCategory(''); setSelectedGender(''); setSelectedShape('');
    setSelectedType(''); setSelectedColor(''); setPriceRange([0, 10000]);
  };

  const activeFilters = [selectedCategory, selectedGender, selectedShape, selectedType, selectedColor]
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <button onClick={() => setShowFilters(!showFilters)}
                ...
                <SlidersHorizontal size={16} /> Filters {activeFilters > 0 && <span className="bg-[#00BAC6] text-white text-xs w-5 h-5 rounded-full flex items-center justify-center">{activeFilters}</span>}
              </button>
              {activeFilters > 0 && (
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="hidden md:flex items-center border border-gray-200 rounded-lg overflow-hidden">
                <button onClick={() => setViewMode('grid')} className={`p-2 ${viewMode === 'grid' ? 'bg-[#000042] text-white' : 'text-gray-400 hover:bg-gray-100'}`}><Grid size={18} /></button>
                <button onClick={() => setViewMode('list')} className={`p-2 ${viewMode === 'list' ? 'bg-[#000042] text-white' : 'text-gray-400 hover:bg-gray-100'}`}><List size={18} /></button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-6">
          ...
          {showFilters && (
            <aside className="hidden md:block w-64 flex-shrink-0">
              <div className="bg-white rounded-xl shadow-md p-5 sticky top-[200px] space-y-6">
                <div>
                  ...
                  {['', 'eyeglasses', 'sunglasses', 'computer-glasses', 'kids', 'reading'].map(cat => (
                    <label key={cat || 'all'} className="flex items-center gap-2 py-1 cursor-pointer text-sm">
                      ...
                      <span className={selectedCategory === cat ? 'text-[#000042] font-semibold' : 'text-gray-600'}>
                        {cat ? cat.replace('-', ' ').replace(/\b\w/g, c => c.toUpperCase()) : 'All'}
                      </span>
                    </label>
                  ))}
                </div>
                  ...
                  {['', 'full-rim', 'half-rim', 'rimless'].map(t => (
                    <label key={t || 'all'} className="flex items-center gap-2 py-1 cursor-pointer text-sm">
                      ...
                      <span className={selectedType === t ? 'text-[#000042] font-semibold' : 'text-gray-600'}>
                        {t ? t.replace('-', ' ').replace(/\b\w/g, c => c.toUpperCase()) : 'All'}
                      </span>
                    </label>
                  ))}
                </div>
          ...
          <main className="flex-1">
            {filteredProducts.length === 0 ? (
              <div className="text-center py-20">
                ...
                <button onClick={clearFilters} className="bg-[#00BAC6] text-white px-6 py-3 rounded-xl font-semibold hover:bg-[#00a8b3] transition-colors">Clear Filters</button>
              </div>
            ) : (
